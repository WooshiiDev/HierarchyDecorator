...
namespace HierarchyDecorator
{
    ...
    internal static class HierarchyDecorator
    {
        ...
        private static Transform finalTransform;
        ...
        private static void DrawElementStyle(GameObject obj, Rect selectionRect)
        {
            ...
            if (hasStyle)
            {
                PrefixSettings style = styles.FirstOrDefault (s => obj.name.TrimStart ().StartsWith (s.prefix));
            ...
            {
                foreach (HierarchyInfo infoGUI in info)
                {
        ...
        {
            ModeOptions styleSetting = prefix.GetCurrentSettings ();
        ...
        private static void ShowChildFoldout(Rect selectionRect, GameObject obj, bool showBack)
        {
            ...
            if (currentTransform == finalTransform && currentTransform.childCount > 0)
            {
            ...
            if (previousTransform.childCount == 0 || previousTransform == finalTransform)
            {
                return;
            }
        ...
        private static bool IsAllowedType(Type type)
        {
            ...
            {
                CustomComponentType component = settings.customComponents[i];
        ...
        private static MonoScript GetCustomType(Type type)
        {
            for (int i = 0; i < settings.customComponents.Count; i++)
            {
                CustomComponentType customType = settings.customComponents[i];
